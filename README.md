...
artifacts/doctavian              canonical provider-native Doctavian template/data
...
The canonical Doctavian inputs live under `artifacts/doctavian/`. The DOCX intentionally preserves the provider-native Maven Word package structure that succeeded against Doctavian's live template reader; do not replace it with a generic DOCX writer without re-running the live provider smoke.

Regenerate/copy the canonical ACME Postman inputs without live credentials:
...
The static web layer may be deployed separately, while the live agent should run as a long-lived Node service because it owns Foxit's stdio MCP child process. `Dockerfile.agent` packages the agent—including the provider-native Doctavian template—for that topology and defaults to fixture mode so live providers are opt-in through runtime secrets.
...
The deterministic workflow, fixture demo, bounded Groq reviewer, Nutrient extraction/Viewer path, Doctavian generation path, Foxit MCP assembly, Foxit eSign handoff, UI, CI safeguards, and deployment packaging are implemented. The current clean-run baseline is **123/123 tests passing**, 0 high-or-greater production dependency vulnerabilities, strict core/provider/app typechecks, a production Vite build, a clean repository secret scan, **2/2 Chromium end-to-end tests passing with retries disabled**, a successful `Dockerfile.agent` build, and a fixture-mode container startup that returns HTTP 200 from `/health`.

Live provider evidence is complete for the real **Groq bounded reviewer**, **Nutrient grounded extraction + DWS Viewer**, **Doctavian structured generation + PDF download**, **Foxit PDF Services MCP**, and the complete **Foxit direct eSign human-signature round trip**. The Doctavian proof reached user preflight `200`, template/data uploads `201`, generation `201`, download `200`, and a rendered VeriRemit PDF containing the canonical ACME case and two verification controls. The Foxit developer-test envelope reached authoritative `EXECUTED` state and provider activity history independently recorded the signer action.

The remaining pre-submission task is not an unresolved sponsor integration: run one fresh canonical end-to-end pass from the exact submitted code immediately before recording, then capture the 2-4 minute demo and submit its public/backup video links.
